...
public class RecordingService : IDisposable
{
    ...
    private string BuildArgs()
    {
        ...
        string videoCodec = "-c:v libx264 -preset ultrafast -tune zerolatency -crf 23 -pix_fmt yuv420p";
    ...
    public void SaveClipAsync()
    {
        Task.Run(() =>
        {
            try
            {
                var segments = Directory.GetFiles(_tempDir, "seg*.ts")
                ...
                var complete = segments.Count > 1 ? segments.Take(segments.Count - 1).ToList() : segments;
                int needed = (int)Math.Ceiling((double)_settings.ClipDurationSeconds / _settings.SegmentDurationSeconds);
                var toMerge = complete.TakeLast(Math.Min(needed, complete.Count)).ToList();

                string listFile = Path.Combine(_tempDir, "concat.txt");
                File.WriteAllLines(listFile, toMerge.Select(f => $"file '{f.FullName.Replace("\\", "/")}'"));

                Directory.CreateDirectory(_settings.OutputFolder);
                string output = Path.Combine(_settings.OutputFolder, $"Clip_{DateTime.Now:yyyy-MM-dd_HH-mm-ss}.mp4");
                ...
                if (proc.ExitCode == 0 && File.Exists(output)) ClipSaved?.Invoke(output);
                else ErrorOccurred?.Invoke("Fehler beim Speichern des Clips.");
            }
